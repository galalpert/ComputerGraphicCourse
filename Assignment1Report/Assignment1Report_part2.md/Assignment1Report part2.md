...
We calculated the translate matrix using the translate function, from glm libaray, and the same for the scaling matrix.
In order to calculate the transformation matrix we multiplied the two matrices we got.
Afterwards, we iterated over all the vertices and made a 4 dimension vector from them, and multiplied by the transformation matrix


## 3.Scale And Translate:
![title](Images/cowMesh.png)
...
## 4.Creating GUI item for mesh transformation:
### Explanation of the solution:
We created 3 sliders for the translation (one for each axis parameter), 3 sliders for the rotation (one for each axis angle) and one slider for the scaling. The slider parameters determine and update the creation of the matrices of each operation.
We created a function in the mesh model, that miltiplies the matrices of each operaion in the following order:
Translation * Scaling * Rotation. Afterwards, we call the function in the renderer and draw the transformed mesh model.

![title](Images/gui.png)
...
## 5. Transform the mesh model using the GUI:
### Translating in the model frame and then rotating in the world frame:
https://user-images.githubusercontent.com/103389828/204615572-a4aebe73-f88e-42ce-ac32-46ea810a260b.mov

### Translating in the world frame and then rotating in the local frame:
https://user-images.githubusercontent.com/103389828/204641785-41d985a9-87c6-4b24-9696-faaad1833969.mov



## 6. Loading several models, switching between the active models and transforming each independently:
https://user-images.githubusercontent.com/103389828/204622356-61cc4951-3772-4747-ae71-2b0553675a97.mov
...
## 7. Implementing two approaches for modifying the basic transformations:
### Using the keyboard:
```cpp
